...
Use the `data` branch of `projects/motion-reference/scenes/ReferenceScene.tsx`, registered as `reference-data`. It includes entrance, reading hold and exit, and imports the shared calibrated profile. The complete implementation is compiled and rendered by `npm run test:render`.

```tsx
const profile = STYLE_PROFILES.data;
