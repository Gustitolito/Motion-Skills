...
Use the `premium` branch of `projects/motion-reference/scenes/ReferenceScene.tsx`, registered as `reference-premium`. It includes entrance, reading hold and exit, and imports the shared calibrated profile. The complete implementation is compiled and rendered by `npm run test:render`.

```tsx
const profile = STYLE_PROFILES.premium;
